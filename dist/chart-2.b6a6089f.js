...
var yPositionScale = d3.scaleLinear().domain([10, 50]).range([height, 0]);
...
Promise.all([d3.csv(require('./data-singleline-cimbria.csv')), d3.csv(require('./data-singleline-cimmeria.csv'))]).then(ready).catch(function (err) {
  console.log('Failed with', err);
});

function ready(_ref) {
  var _ref2 = _slicedToArray(_ref, 2),
      datapointsCimbria = _ref2[0],
      datapointsCimmeria = _ref2[1];

  console.log(datapointsCimbria);

  /* Add in your temperature circles */
  svg.selectAll('.cimbria-dot').data(datapointsCimbria).enter().append('circle').attr('class', 'cimbria-dot').attr('r', 3).attr('cx', function (d) {
    return xPositionScale(d.day);
  }).attr('cy', function (d) {
    return yPositionScale(d.temperature);
  });

  svg.selectAll('.cimmeria-dot').data(datapointsCimmeria).enter().append('circle').attr('class', 'cimmeria-dot').attr('r', 3).attr('cx', function (d) {
    return xPositionScale(d.day);
  }).attr('cy', function (d) {
    return yPositionScale(d.temperature);
  });
  ...
  svg.append('path').datum(datapointsCimbria).attr('stroke-width', 2).attr('stroke', 'lightblue').attr('fill', 'none').attr('d', line).lower();

  svg.append('path').datum(datapointsCimmeria).attr('stroke-width', 2).attr('stroke', 'pink').attr('fill', 'none').attr('d', line).lower();
...
},{"d3":"../node_modules/d3/index.js","./data-singleline-cimbria.csv":"data-singleline-cimbria.csv","./data-singleline-cimmeria.csv":"data-singleline-cimmeria.csv"}],"../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js","chart-2.js"], null)
//# sourceMappingURL=/chart-2.b6a6089f.map